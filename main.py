...
client = discord.Client(intents=intents,
                        activity=discord.Game(name='Use .bd .dw .ca'))
...
async def get_timers():
  ...
  return r
# Run coroutine using run_until_complete
...
BD1_Status1 = []
ABBA1_Time = datetime.now(utc)
ABBA1_Status1 = []

babies_respawn_time = datetime.now(utc)
...
def get_next_dw():
  ...
  dw_timers = ['01', '04', '07', '10', '13', '16', '19', '22']


  for x in dw_timers:
    if(now.time() < time.fromisoformat(x + ':40:00')):
      get_timer = x
      dw_isnextday = False
      break
    else:
...
dwnp_isnextday = False
def get_next_dwnp():
  global now
  global dwnp_isnextday
  
  now = datetime.now(utc)
  #now = datetime.fromisoformat('2022-10-22 00:00:00.000000+00:00')
  dwnp_timers = ['00', '03', '06', '09', '12', '15', '18', '21']


  for x in dwnp_timers:
    if(now.time() < time.fromisoformat(x + ':40:00')):
      get_timer = x
      dwnp_isnextday = False
      break
...
async def on_ready():
    ...
    #maze_timers_const()  #update maze start time
...
`.list                `
    ...
    if content.startswith(
      ...
      await channel.send("This timer is fucked up for now, please check PKO Timers instead: https://pirateking.online/database/portal")
    '''
        # BD TIMER
    # THIS IS THE CODE THAT WEBSCRAPE BD TIMER
    if content.startswith(
      'timer bd2') or content.startswith(
      'what time bd2') or content.startswith(
      '.bd2') or content.startswith(
      'when bd2') or content.startswith(
      'timer pr bd2') or content.startswith(
      'what time pr bd2') or content.startswith(
      '.pr bd2') or content.startswith(
      'when pr bd2'):
        ...
        if (missing_time < timedelta(seconds=1) or BD1_Status1 == 'Alive'):
            ...
            missing_time = BD1_Time - now
            if (BD1_Status1 == 'Alive'):
                await channel.send('**BD is ALIVE !!!!**')
        else:
            msg1 = '**BD** Spawns in '
            ...
            await channel.send(msg1 + time_left + '\n' + spawntime + ' ' + userutc +
                               ' UTC Timezone.**')
      #HERE ENDS THE CODE THAT WEBSCRAP BD TIME
    ...
    if content.startswith(
      ...
      '.pr bd') or content.startswith(
      'when pr bd'):
      await channel.send("Free farmers")
      
      '''
        now = datetime.now(utc)
        missing_time = eventlist.bd_start.replace(tzinfo=utc) - now
        
        userutc = get_user_utc(content)

        if (missing_time < timedelta(seconds=1)):
            await channel.send('**BD is ALIVE !!!!** (if its not alive, DM LuisaRLZ#3429 for update timer.)')
        else:
            msg1 = '**BD** Spawns in '

            time_left = get_time_left(missing_time)

            BD1_Timeutc = eventlist.bd_start.replace(tzinfo=utc) + timedelta(hours=int(userutc))
            spawntime = BD1_Timeutc.strftime('%A, %B %d | **%H:%M')
            if (userutc == '+0'):
                userutc = ''
            await channel.send(msg1 + time_left + '\n' + spawntime + ' ' + userutc +
    ...
    'when np bd'):
      ...
      now = datetime.now(utc)
      missing_time = eventlist.npbd_start.replace(tzinfo=utc) - now

      userutc = get_user_utc(content)

      if (missing_time < timedelta(seconds=1)):
          await channel.send('**NEW PROVIDENCE BD is ALIVE !!!!** (if its not alive, DM luisarlz or gaius98 for update timer.)')
      else:
          msg1 = '**(NEW PROVIDENCE) :dragon: BD** Spawns in '

          time_left = get_time_left(missing_time)

          BD1_Timeutc = eventlist.npbd_start.replace(tzinfo=utc) + timedelta(hours=int(userutc))
          spawntime = BD1_Timeutc.strftime('%A, %B %d | **%H:%M')
          if (userutc == '+0'):
              userutc = ''
          await channel.send(msg1 + time_left + '\n' + spawntime + ' ' + userutc +
    ...
    # SET BABIES TIMER
    if content.startswith(
      '.set babies') or content.startswith(
      '.set baby'):
      
      print(message.author)
      if message.author.id == 170777014238707713:
        await channel.send('you suck')
        return
      
      respawn_cd = 1200 #seconds
      now = datetime.now(utc)
      babies_respawn_time = now + timedelta(seconds=respawn_cd)
      confirmationtext = 'Babies will spawn at ' + babies_respawn_time.strftime('%A, %B %d | **%H:%M UTC**')
      await channel.send('.timer 15m {0.author.mention} babies spawn in 5 minutes'.format(message))
      await channel.send(confirmationtext)
      

    # BABIES TIMER
    if content.startswith(
      '.babies') or content.startswith(
      '.baby'):
      ...
      time_left = babies_respawn_time - now

      if(time_left.total_seconds() > 0):
        time_left_float = time_left.total_seconds()
        minutes, seconds = divmod(time_left_float, 60)
        time_left_string = ('**{:02}m {:02}s**'.format(int(minutes), int(seconds)))
        
        string_respawn1 = 'Babies will spawn in ' + time_left_string + '.\n'
        string_respawn2 = '**Spawn Time:** ' + babies_respawn_time.strftime('%H:%M UTC')
        
        await channel.send(string_respawn1 + string_respawn2)
        
      else:
        await channel.send("**Baby timer is gone.**")
    ...
    if content.startswith(
      ...
      'when fc') or content.startswith(
      'when ds') or content.startswith(
      'timer dw') or content.startswith(
      'what time dw') or content.startswith(
      '.dw') or content.startswith(
      'when dw'):

      await channel.send("FC/DS/DW timers are under maintenance.")
      ...
      next_dw = datetime.fromisoformat (now.date().isoformat() + ' ' + get_next_dw() + ':00:00')
      if(dw_isnextday == True):
        next_dw = next_dw + timedelta(days=1)
      ...
      if(ishappening == True):
        msg1 = '**(PORT ROYAL) DW / FC / DS ARE OPEN RIGHT NOW!!**'
        msg2 = '\n(PORT ROYAL) Portal will close in '
        await channel.send(msg1 + msg2 + time_left)
      ...
      if(ishappening == False):
        ...
        msg1 = '**(PORT ROYAL) DW / FC / DS** portals open in '
    ...
    if content.startswith(
      ...
      #print(next_aada)
      #print("aurora first?" + str(aurora_first))
